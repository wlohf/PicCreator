...
set "ROOT=%~dp0"
set "APP_DIR=%ROOT%attuno-studio"
...
echo [INFO] Starting Attuno Studio split app ...
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  $healthOk=$false;" ^
  "  try { $health=Invoke-RestMethod -Uri ('http://127.0.0.1:' + $port + '/api/health') -TimeoutSec 2; $healthOk=($health.service -eq 'attuno-studio-api') } catch {}" ^
  "  $own=@(); $foreign=@();" ^
  ...
  "  Write-Host ('[INFO] Stopped ' + $own.Count + ' old Attuno Studio API process(es) on port ' + $port + '.'); exit 0" ^
  "}"
if errorlevel 2 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  foreach($owner in $own) { Stop-Process -Id $owner -Force }" ^
  "  Start-Sleep -Milliseconds 800;" ^
  "  Write-Host ('[INFO] Stopped ' + $own.Count + ' old Attuno Studio Web process(es) on port ' + $port + '.'); exit 0" ^
  "}"
if errorlevel 2 (
...
start "Attuno Studio API" cmd /k "cd /d ""%APP_DIR%"" && set APP_HOST=%APP_HOST%&& set APP_PORT=%APP_PORT%&& ""%PY%"" api_server.py"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "    if($health.service -eq 'attuno-studio-api') { $ok=$true; break }" ^
  "  } catch {}" ^
  "  Start-Sleep -Seconds 1" ^
  "}" ^
  "if($ok) { Write-Host ('[INFO] API is ready on http://127.0.0.1:' + $port + '/api/health'); exit 0 }" ^
  "Write-Host ('[ERROR] API did not become ready on http://127.0.0.1:' + $port + '/api/health within 60 seconds. Check the Attuno Studio API window for errors.'); exit 1"
if errorlevel 1 (
  pause
  exit /b 1
)

start "Attuno Studio Web" cmd /k "cd /d ""%UI_DIR%"" && set VITE_PORT=%VITE_PORT%&& set VITE_API_TARGET=%VITE_API_TARGET%&& npm run dev -- --host 0.0.0.0 --port %VITE_PORT% --strictPort"
