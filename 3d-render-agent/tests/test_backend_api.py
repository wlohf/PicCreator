...
def test_health_endpoint_reports_api_status():
    ...
    assert payload["service"] == "3d-render-agent-api"
...
def test_token_namespace_config_save_and_load_are_isolated(tmp_path, monkeypatch):
    ...
    alpha_headers = {"X-Render-Agent-User-Token": "alpha-token"}
    beta_headers = {"X-Render-Agent-User-Token": "beta-token"}
    response = client.post(
        "/api/config/save",
...
def test_relative_data_dir_is_project_root_based(monkeypatch):
    ...
    monkeypatch.setenv("RENDER_AGENT_DATA_DIR", "relative-data")
    project_root = Path(__file__).resolve().parents[1]
...
def test_token_namespace_isolates_unauthenticated_results(tmp_path, monkeypatch):
    ...
    alpha_response = client.get("/api/results", headers={"X-Render-Agent-User-Token": "alpha-token"})
