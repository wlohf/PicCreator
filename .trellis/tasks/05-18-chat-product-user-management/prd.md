...
* 后端已有真实账号能力：`/api/auth/register`、`/api/auth/login`、`/api/auth/logout`、`/api/auth/me`，主 session cookie 名为 `attuno_session`；`render_agent_session` 仅作为旧版兼容别名保留。
* 前端也已有 `auth.ts` API 封装，但主界面仍使用“临时访问标识”弹窗，并明确提示“不是安全登录”。
* `apiFetch` 当前依赖 `credentials: "include"` 携带登录 session；后端 `get_current_or_default_user()` 必须先解析真实 session，再把 `X-Attuno-User-Token` 或旧版 `X-Render-Agent-User-Token` 当作未登录兼容 namespace。
...
* 用户管理：
  ...
  * 后端可以保留 token namespace 兼容测试/旧调用，但真实登录 session 必须优先，已登录请求不能被 `X-Attuno-User-Token` 或旧版 `X-Render-Agent-User-Token` 覆盖到任意 namespace。
...
* [ ] 已登录用户请求不会被 `X-Attuno-User-Token` 或旧版 `X-Render-Agent-User-Token` 覆盖到任意 namespace。
...
* Existing auth backend:
  ...
  * `attuno-studio/backend/app/routes/auth.py`
...
* Frontend identity flow:
  ...
  * `attuno-studio/ui-prototype/src/api/auth.ts`
  * `attuno-studio/ui-prototype/src/api/client.ts`
* Memory/preferences:
  * `attuno-studio/backend/app/services/preferences_store.py`
  * `attuno-studio/backend/app/services/design_chat_agent.py`
  * `attuno-studio/backend/app/routes/chat.py`
  * `attuno-studio/backend/app/routes/preferences.py`
  * `attuno-studio/ui-prototype/src/api/chat.ts`
  * `attuno-studio/ui-prototype/src/api/preferences.ts`
* Relevant specs:
